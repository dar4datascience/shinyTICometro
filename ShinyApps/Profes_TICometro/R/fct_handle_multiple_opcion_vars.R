...
handle_multiple_choice_questions <- function(ticometro_table, select_schools, select_groups = NULL, multiple_opcion_var, grouping_var = NULL, fecha_aplicacion = 2020){
  ...
  if (is.null(select_groups) | (any(stringr::str_detect(select_schools, "ENP"))
    ...
    if(is.null(grouping_var)){
      ...
      if (multiple_opcion_var == "dispositivos_electronicos"){
        ...
        tidy_multiple_choice_directivos_cch <- count_var_4_directivos_and_cch %>% 
          mutate(
            dispositivos_electronicos = stringr::str_replace_all(dispositivos_electronicos, "_", " "),
            dispositivos_electronicos = stringr::str_replace_all(dispositivos_electronicos, "sin responder dispositivo x estudiante", "No declaró")
          )
    ...
    }else{ #logic for a 2 grouping variable
      ...
      if (multiple_opcion_var == "dispositivos_electronicos"){
        ...
        tidy_2_vars_multiple_choice_directivos_cch <- count_2vars_4_directivos_and_cch %>% 
          mutate(
            dispositivos_electronicos = stringr::str_replace_all(dispositivos_electronicos, "_", " "),
            dispositivos_electronicos = stringr::str_replace_all(dispositivos_electronicos, "sin responder dispositivo x estudiante", "No declaró")
          )
    ...
    #END DIRECTIVO Y CCH LOGIC
  } else{  #logic 4 ENP
    
    #handle single count
    if(is.null(grouping_var)){
      
      count_var_4_ENP <- ticometro_table %>%  #A conection pointing to the ticometro table
        filter(fecha_aplicacion == fecha_aplicacion,
      ...
      count_2vars_4_ENP <- ticometro_table %>%  #A conection pointing to the ticometro table
        filter(fecha_aplicacion == fecha_aplicacion,
               institucion %in% select_schools,
               grupo %in% select_groups) %>%
