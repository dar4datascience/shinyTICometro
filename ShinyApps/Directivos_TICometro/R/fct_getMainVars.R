#' get_mainVars_4_planteles: A function to get main vars 
#' from the ticometro filtering escuela
#' @description ABOUT POOL connection:First, you should create your
#' pool at the top of server.R (or in global.R),
#' but outside the actual server function.
#' Then, for each query, you should use tbl (or equivalent).
#' In a Shiny app, this means that each reactive or function
    ...
    if (fecha_de_aplicacion == "2020") {
      ...
      if (is.null(select_groups) |
        ...
        enp_main_vars <- ticometro_table %>%
          ...
          filter(
            institucion %in% select_schools, # filtra por escuela
            grupo %in% select_groups
          ) %>% # filtra el fecha_aplicacion de aplicacion
          collect() # pull data into the applicacion
